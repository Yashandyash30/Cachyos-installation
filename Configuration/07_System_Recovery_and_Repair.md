# 07 - System Recovery and Repair Guide

This document aggregates emergency recovery steps for when your system breaks, specifically focusing on boot loops, black screens, and display manager (SDDM) failures on CachyOS/BTRFS.
...
You are back in, but the system is still vulnerable. If a custom script accidentally stripped out Wayland UI dependencies, the next time you run `sudo pacman -Syu`, SDDM will crash all over again. We need to bulletproof the login screen.
