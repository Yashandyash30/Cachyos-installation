...
4. Move the compiled binary to your local binaries folder so it doesn't overwrite your stable CachyOS Niri installation:
...
1. Open the Sunshine Web UI (`https://localhost:47990`) and navigate to **Application -> edit (for first Desktop)**.
2. cahnge the name from **Desktop -> Desktop-Privacy**
3. Set the **Do Command** (below command Preperation) to spin up the virtual screen and put the physical monitors to sleep:
...
3. Set the **Undo Command** to wake your physical monitors and destroy the virtual output upon disconnect:
...
click on **Save** at the bottom to apply changes.

To add another normal Desktop entry

1. Click on **Add New**.
2. Enter **Desktop** in **Application Name** box.
3. Click on **Find Cover** in Image Section to choose appropriate icon for the desktop.
4. Click on **Save** at the bottom to apply changes.
